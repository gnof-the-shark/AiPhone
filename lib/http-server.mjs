...
import { generateGeminiResponse } from './gemini.mjs';
 ...
 * @param {Function}      [_deps._generateGeminiResponse] – override Gemini response generator
 * @returns {Promise<http.Server>}
 */
export async function createServer(config, { _request, _generateGeminiResponse } = {}) {
  const {
    PORT,
    API_TOKEN,
    CLAWPHONE_API_KEY,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    log.log(`incoming request: ${req.method} ${u.pathname}`);
    ...
    // ── POST /v1/sms/receive ───────────────────────────────────────────────
    if (req.method === 'POST' && u.pathname === '/v1/sms/receive') {
      let body;
      try {
      ...
      if (!from || !to || !userText) {
        sendJson(res, 400, { error: 'Missing From, To or Body' });
        return;
      }

      log.log('inbound sms', { from, to, bodyLength: userText.length });
      ...
      if (!GEMINI_API_KEY) {
        log.warn('GEMINI_API_KEY not set, responding with error message');
        res.writeHead(200, { 'Content-Type': 'application/xml' });
        res.end(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>Désolé, je ne suis pas configuré pour répondre pour le moment.</Message></Response>`);
        return;
      }

      try {
        const generate = _generateGeminiResponse || generateGeminiResponse;
        const reply = await generate({
          apiKey: GEMINI_API_KEY,
          model: GEMINI_MODEL,
          prompt: `Tu es un assistant IA serviable qui répond par SMS. Sois concis (max 160 caractères si possible). L'utilisateur avec le numéro ${from} te dit : ${userText}`,
        });
        ...
        res.writeHead(200, { 'Content-Type': 'application/xml' });
        res.end(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>Désolé, une erreur est survenue lors de la génération de ma réponse.</Message></Response>`);
      }
      return;
    }
